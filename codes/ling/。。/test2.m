function [fc,nextu,stretchedu,k] = test(u0,a,w,lamda, dt, threshold)
...
%%for findex = 1:size(fc)
   ...
   nextu = min(1,max(0,thisu + dt*(-A'*(A*thisu-fc)-(lamda*thisu))));
    iterations = 1;
    while ((norm(nextu - thisu)/norm(thisu) > threshold) && (iterations < 1000))
       thisu = nextu;
       nextu = min(1,max(0,thisu + dt*(-A'*(A*thisu-fc)-(lamda*thisu))));
      
       if mod(iterations,10) == 0
           display(['Going.............................................' num2str(iterations), ' iterations'])%iterations)
       end
       iterations = iterations + 1;
    end
    display(iterations)
stretchedu = stretchedu(cutoff+1:end-cutoff);    
%%end
end
